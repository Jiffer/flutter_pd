...
class RequestPermissionHandler(private val callback: (Boolean) -> Unit) : PluginRegistry.RequestPermissionsResultListener {
  ...
  fun requestPermission(activity: Activity): Boolean {
    ...
    return if (ContextCompat.checkSelfPermission(activity, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
      ActivityCompat.requestPermissions(activity, arrayOf(Manifest.permission.RECORD_AUDIO), RECORD_PERMISSION_REQUEST)
      true
    } else {
